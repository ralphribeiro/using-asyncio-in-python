...
async def client(reader: StreamReader, writer: StreamWriter):
    ...
    # - On first connect, a client must send a message containing the channel
    #   to subscribe to (here, subscribe_chan).
    # - There after, for the life of the connection, a client sends a message
    ...
    SUBSCRIBES[subscribe_chan].append(writer)
    ...
    try:
        ...
        while channel_name := await read_msg(reader):
            ...
            # all of the sending to complete.
            # This line is a bad flaw in our program, but it may not be obvious
            # why: though it may be true that all of the sending to each
            # subscriber will happen concurrently, what happens if we have one
            # very slow client? In this case, the gather() will finish only
            # when the slowest subscriber has received its data. We can’t
            # receive anymore data from the sending client until all these
            # send_msg() coroutines finish. This slows all message distribution
            # to the speed of the slowest subscriber.
        ...
        # When leaving the client() coroutine, we make sure to remove ourselves
        # from the global SUBSCRIBERS collection. Unfortunately, this is an
        # O(n) operation, which can be a little expensive for very large n.
        # A different data structure would fix this, but for now we console
        # ourselves with the knowledge that connections are intended to be
        # long-lived (thus, there should be few disconnection events) and n is
        # unlikely to be very large (say ~10,000 as a rough order-of-magnitude
        # estimate), and this code is at least easy to understand.
        SUBSCRIBES[subscribe_chan].remove(writer)
