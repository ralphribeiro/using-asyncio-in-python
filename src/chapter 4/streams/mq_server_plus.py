# Message broker: improved design
import asyncio
...
CHAN_QUEUES: dict[bytes, Queue] = dict()
...
async def client(reader: StreamReader, writer: StreamWriter):
    ...
    try:
        while channel_name := await read_msg(reader):
            ...
            # Now we’re inside the loop where we receive data. Remember that we
            # always receive two messages: one for the destination channel
            ...
            # channel, we’re going to put that data on to the appropriate queue
            ...
            if channel_name not in CHAN_QUEUES:
                ...
                # coroutinechan_sender() will be responsible for taking data
                # off the channel queue and distributing that data to subscribers.
            ...
            # the new data. Waiting here means we won’t be reading any new data
            # off the socket, which means that the client will have to wait on
            # sending new data into the socket on its side. This isn’t necessarily
            # a bad thing, since it communicates so -called back- pressure to this
            # client. (Alternatively, you could choosWe to drop messages here if
            # the use case is OK with that.)
        ...
        # next line, we also remove the sock from the SUBSCRIBERS collection as
        # before).
    ...
    with suppress(asyncio.CancelledError):
        while True:
            ...
            if not writers:
                ...
                # sends data from a dedicated channel Queue instance to all the
                # subscribers on that channel. But what happens if there are no
                # subscribers for this channel yet? We’ll just wait a bit and
                # try again. (Note, though, that the queue for this channel,
                # CHAN_QUEUES[name], will keep filling up.)
                continue

            # As in our previous broker implementation, we do something special
            # for channels whose name begins with /queue: we rotate the deque
            # and send only to the first entry. This acts like a crude
            # load-balancing system because each subscriber gets different
            # messages off the same queue. For all other channels, all
            # subscribers getall the messages.
            ...
            # chan_sender() coroutines live forever), but iflogic were added
