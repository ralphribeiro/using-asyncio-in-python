...
async def main(args):
    ...
    try:
        # Using itertools.count() is like a while True loop, except that we get
        # an iteration variable to use. We use this in the debugging messages
        # since it makes it a bit easier to track which message got sent from
        # where.
        for i in count():
            # The delay between sent messages is an input parameter,
            # args.interval. The next line generates the message payload. It’s
            # either a byte string of specified size(args.size) or a
            # descriptive message. This flexibility is just for testing.
            await asyncio.sleep(args.interval)
            data = b'X'*args.size or f'Msg {i} for {me}'.encode()
            try:
