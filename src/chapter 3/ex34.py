...
def handler(sig):
    # The primary purpose of the handler is to stop the loop: this will unblock
    # the loop.run_forever() call and allow pending task collection and
    # cancellation, and the run_complete() for shutdown.
    loop.stop()
    ...
    # during therun_until_complete() phase, which would interfere with our
    ...
    # for SIGINT, the same as it was before we added our own handlers. Instead,
    # we set an empty lambda function as the handler. This means that
